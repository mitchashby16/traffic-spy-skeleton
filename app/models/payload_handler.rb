module TrafficSpy
  class PayloadHandler
    ...
    def call
      ...
      elsif duplicate_payload?(parse_payload(payload))
        @status = 403
        @body = "Already Received Request - 403 Forbidden"
      else
        save_data(identifier, parse_payload(payload))
        @status = 200
        @body = "Success"
      end
    end
    ...
    def parse_payload(input)
      convert_keys_to_symbols(convert_keys_to_snakecase(payload_to_hash(input)))
    end

    def payload_to_hash(input)
      if input
        JSON.parse(input)
      else
        {}
